...
void testBasicAllocation() {
    ...
    assert(ptr1 != nullptr);
    MemoryPool::release(ptr1, 8);

    void* ptr2 = MemoryPool::allocate(1024);
    assert(ptr2 != nullptr);
    MemoryPool::release(ptr2, 1024);

    void* ptr3 = MemoryPool::allocate(1024 * 1024);
    assert(ptr3 != nullptr);
    MemoryPool::release(ptr3, 1024 * 1024);
        ...
        try {
            ...
            for(size_t i = 0; i < ALLOCS_PER_THREAD && !has_error; i ++) {
                ...
                if(rand() % 2 && !allocations.empty()) {
                    ...
                    MemoryPool::release(allocations[index].first, allocations[index].second);
                    allocations.erase(allocations.begin() + index);
                }
            }

            for(const auto& alloc : allocations) {
                MemoryPool::release(alloc.first, alloc.second);
            }
