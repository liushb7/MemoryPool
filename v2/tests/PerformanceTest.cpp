...
public:
    ...
    static void warmup() 
    {
        ...
        {
            MemoryPool::release(ptr, size);  // 使用实际分配的大小进行释放
        }
    ...
    static void testSmallAllocation() 
    {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) 
            {
                ...
                if (i % 4 == 0) 
                {
                    MemoryPool::release(ptrs.back(), SMALL_SIZE);
                    ptrs.pop_back();
                }
            }
            ...
            for (void* ptr : ptrs) 
            {
                MemoryPool::release(ptr, SMALL_SIZE);
            }
        ...
        auto threadFunc = [](bool useMemPool) 
        {
            ...
            for (size_t i = 0; i < ALLOCS_PER_THREAD; ++i) 
            {
                ...
                {  // 75%的概率释放
                    ...
                    if (useMemPool) 
                    {
                        MemoryPool::release(ptrs[index].first, ptrs[index].second);
                    } 
                    else 
                    {
        ...
        // 测试内存池
        {
            ...
            for (size_t i = 0; i < NUM_ALLOCS; ++i) 
            {
                ...
                if (i % 100 == 0 && !ptrs.empty()) 
                {
                    ...
                    {
                        MemoryPool::release(ptrs.back().first, ptrs.back().second);
                        ptrs.pop_back();
                    }
                }
            }
